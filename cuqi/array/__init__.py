...
def _load_backend():
    ...
    if _BACKEND_NAME in ["numpy", "np"]:
        ...
        from ._cupy import load_backend
        return load_backend()
    elif _BACKEND_NAME in ["jax", "jnp"]:
        from ._jax import load_backend
        return load_backend()
    else:
...
def _expose_backend_functions():
    ...
    if _BACKEND_NAME in ["numpy", "np"]:
        ...
        from ._cupy import get_backend_functions, to_numpy, pad
    elif _BACKEND_NAME in ["jax", "jnp"]:
        from ._jax import get_backend_functions, to_numpy, pad
    else:
        raise ValueError(f"Unsupported backend: {_BACKEND_NAME}")
...
__all__ = [
    ...
    'asarray', 'asanyarray', 'ascontiguousarray', 'asfortranarray', 'copy',
     ...
     'finfo', 'iinfo', 'newaxis', 'inf', 'nan', 'pi', 'e',
