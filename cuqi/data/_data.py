...
def threephases(size=128, p=70):
    ...
    x1     = xp.zeros((N,N))
    for i in range(p):
        x1 += xp.exp(-xp.abs(I-c1[i,0])**3/(2.5*sigma1)**3 - xp.abs(J-c1[i,1])**3/sigma1**3)
    t1 = 0.35
    x1[x1 < t1]  = 0
    x1[x1 >= t1] = 2
...
def p_power(size=128, relnz=0.3, p=2, seed=1): #relnz=0.65, p=2.3
    ...
    F = xp.abs(xp.fft.ifft2(F))
    f = -xp.sort(-F.flatten(order='F'))   # 'descend'
    k = xp.round(relnz*N**2)-1
    F[F < f[k]] = 0
    x = F/f[0]
    if Nodd:
        x = F[1:-1,1:-1]
...
def imresize(image, size, **kwargs):
    ...
    return xp.ndimage.zoom(image, zoom_factor, **kwargs)
