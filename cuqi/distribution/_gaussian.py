...
class Gaussian(Distribution):
    ...
    def _sample(self, N=1, rng=None):
        ...
        if spa.issparse(self.sqrtprec): # do sparse
            if (N == 1):
                perturbation = spa.linalg.spsolve(self.sqrtprec, e)[:, xp.newaxis]
            else:
    ...
    # cov is full
        ...
        if spa.issparse(cov):
            ...
            if sparse_flag:
                ...
                d = s[s > eps]                    
                s_pinv = xp.array([0 if xp.abs(x) <= eps else 1/x for x in s], dtype=float)
...
def eigvalsh_to_eps(spectrum, cond=None, rcond=None):
    ...
    eps = cond * xp.max(xp.abs(spectrum))
    return eps
